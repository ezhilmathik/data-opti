...
int main(int argc, char *argv[]) {
    ...
    int AtoH = 0, BtoH = 1, CtoH = 2, DtoH = 3;
    omp_depend_t depA, depB, depC, depD;
    #pragma omp depobj(depA) depend(out: AtoH)
    #pragma omp depobj(depB) depend(out: BtoH)
    #pragma omp depobj(depC) depend(out: CtoH)
    #pragma omp depobj(depD) depend(out: DtoH)

    // ── Warm-up transfers (not timed, synchronous D→H) ────────────────────────
    for(int w=0; w<2; w++)
      {
	omp_target_memcpy_async(h_A, d_A, bytes, 0, 0, host_id, dev0, 1, &depA);
	omp_target_memcpy_async(h_B, d_B, bytes, 0, 0, host_id, dev1, 1, &depB);
	omp_target_memcpy_async(h_C, d_C, bytes, 0, 0, host_id, dev2, 1, &depC);
	omp_target_memcpy_async(h_D, d_D, bytes, 0, 0, host_id, dev3, 1, &depD);
#pragma omp taskwait depend(depobj: depA, depB, depC, depD)
      }
#pragma omp taskwait depend(depobj: depA, depB, depC, depD)
    
    // ── Timed transfer (async D→H via omp_target_memcpy_async) ───────────────
