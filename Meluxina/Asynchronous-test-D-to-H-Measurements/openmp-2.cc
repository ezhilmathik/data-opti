...
#pragma omp taskwait depend(depobj: depA, depB)
      }
#pragma omp taskwait depend(depobj: depA, depB)
    ...
    // Wait until both async copies complete before stopping timer
    #pragma omp taskwait depend(depobj: depA, depB)
    ...
    cout << "Verification h_B (device " << dev1 << " → host): "
         << (passB ? "PASSED" : "FAILED")
         << " (" << sample_size << "/" << N << " samples)\n";
