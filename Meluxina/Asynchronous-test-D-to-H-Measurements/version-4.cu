...
int main(int argc, char *argv[]) {
	...
	checkCuda(cudaSetDevice(2), "cudaSetDevice 2");
	checkCuda(cudaMemcpyAsync(h_C, d_C, bytes, cudaMemcpyDeviceToHost, stream[2]), "warmup D→H d_C");
	checkCuda(cudaStreamSynchronize(stream[2]), "cudaStreamSynchronize warmup stream[2]");
	checkCuda(cudaSetDevice(3), "cudaSetDevice 3");
	checkCuda(cudaMemcpyAsync(h_D, d_D, bytes, cudaMemcpyDeviceToHost, stream[3]), "warmup D→H d_D");
    ...
    #pragma omp parallel sections
    {
        ...
        {
            checkCuda(cudaSetDevice(2), "cudaSetDevice 2");
            checkCuda(cudaMemcpyAsync(h_C, d_C, bytes, cudaMemcpyDeviceToHost, stream[2]), "timed cudaMemcpyAsync D→H d_C");
            checkCuda(cudaStreamSynchronize(stream[2]), "cudaStreamSynchronize timed stream[2]");
        }
        #pragma omp section
        {
            checkCuda(cudaSetDevice(3), "cudaSetDevice 3");
            checkCuda(cudaMemcpyAsync(h_D, d_D, bytes, cudaMemcpyDeviceToHost, stream[3]), "timed cudaMemcpyAsync D→H d_D");
