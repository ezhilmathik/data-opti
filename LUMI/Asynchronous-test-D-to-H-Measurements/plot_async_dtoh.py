#!/usr/bin/env python3
"""
plot_async_dtoh.py — Bandwidth vs N for versions v1, v2, v4 (Async D-to-H transfer)
...
# Best-fit theoretical line for your data (~40.6 max)
...
def main():
    ...
    for key in SERIES_ORDER:
        ...
        line, = ax2.plot(xs, bws, marker=marker,
    ...
    if THEORETICAL_BW is not None:
        ...
        ax2.text(x_pos[mid], THEORETICAL_BW + 0.4,
                 f"Theoretical BW ({THEORETICAL_BW} GB/s)",
                 color="gray", ha="center", va="bottom", fontsize=12)

    ax2.set_ylabel("Bandwidth (GB/s)", fontsize=20)
    ax2.tick_params(axis="y", labelsize=16)

    max_bw = max(bw for (_, _), nd in data.items() for bw in nd.values())
    ymax = max(max_bw * 1.10, THEORETICAL_BW * 1.08)
    ax2.set_ylim(0, ymax)

    fig.legend(handles=series_handles,
               loc="upper left", bbox_to_anchor=(0.12, 0.92),
               fontsize=12)

    plt.tight_layout()

    out_path = os.path.join(OUTPUT_DIR, "async_dtoh_bandwidth.pdf")
